...
from ..utils.connectome_utils import vectorize_symmetric_list
...
def sample_injury_strengths(n_samples, X_mn, A, B, noise_weight):
    '''Returns a n connectomes with simulated injury from two sources.'''
    mult_factor = 10
...
def load_base_connectome(file_name, verbose=False):
    ...
    if verbose:
        ...
        # plt.imshow(X_mn, interpolation="none"); plt.colorbar(); plt.title('Averaged normalized connectome')
...
def generate_injury_signatures(X_mn, n_injuries):
    ...
    for idx, sig_idx in enumerate(sig_indexes):
        ...
        # plt.subplot(1,len(sig_indexes),idx+1); plt.imshow(A, interpolation="none"); plt.colorbar(fraction=0.045, pad=0.04);
        # plt.subplot(1,2,2); plt.imshow(B, interpolation="none"); plt.colorbar(fraction=0.045, pad=0.04);
